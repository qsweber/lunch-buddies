...
class SlackClient(object):
    ...
    def open_conversation(self, bot_access_token: str, **kwargs):
        return self._get_base_client_for_token(bot_access_token).api_call('conversations.open', **kwargs)

    def post_message(self, bot_access_token: str, **kwargs):
        return self._get_base_client_for_token(bot_access_token).api_call('chat.postMessage', **kwargs)
    ...
    def _users_info_internal(self, bot_access_token: str, **kwargs) -> dict:
        return self._get_base_client_for_token(bot_access_token).api_call('users.info', **kwargs)
    ...
    def get_user_name_email(self, bot_access_token: str, user_id: str) -> Tuple[str, str]:
        user_info = self._users_info_internal(bot_access_token, user=user_id)
